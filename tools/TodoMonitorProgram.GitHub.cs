...
internal static class Program
{
    ...
    public static int Main(string[] args)
    {
        ...
        if (!GitRepository.TryFindRepositoryRoot(options.MonitorFolder!, out var repositoryRoot))
        {
        ...
        if (options.CreateGitHubIssues && !GitHubIssueCreator.IsGhAvailable(repositoryRoot))
        {
        ...
        var issueCreator = new GitHubIssueCreator(options.CreateGitHubIssues, repositoryRoot);
        var processor = new TodoProcessor(options.TodoMarkdownFile!, issueCreator);
        ...
        Console.WriteLine($"Git root:  {repositoryRoot}");
...
internal sealed class CommandLineOptions
{
    ...
    public static string Usage =>
        ...
        Requirements:
          ...
          - Use --no-github to only normalise JavaScript Todos and update the markdown file.
...
internal sealed class TodoProcessor
{
    ...
    public TodoProcessor(string todoMarkdownFile, GitHubIssueCreator issueCreator)
    {
        ...
        out ParsedTodo parsed)
    {
        ...
        var label = fields.TryGetValue("Label", out var labelValue)
            ? labelValue
            : string.Empty;
    ...
    private static string FormatFullComment(TodoItem item, string indent, string newLine)
    {
        ...
        lines.AddRange(FormatCommentField(indent, "Description", item.Description, newLine));
    ...
    private static IEnumerable<string> FormatCommentField(string indent, string name, string value, string newLine)
    {
...
internal sealed class GitHubIssueCreator
{
    ...
    public GitHubIssueCreator(bool enabled, string repositoryRoot)
    {
        Enabled = enabled;
    ...
    private static string CreateIssueBody(TodoItem item)
    {
...
{
    public static bool TryFindRepositoryRoot(string folder, out string repositoryRoot)
    {
        ...
        var gitResult = ExternalCommand.Run(
            "git",
            new[] { "-C", fullFolder, "rev-parse", "--show-toplevel" },
            fullFolder,
            TimeSpan.FromSeconds(10));
        ...
        return TryFindRepositoryRootByWalking(fullFolder, out repositoryRoot);
    }
...
internal readonly record struct JavaScriptComment(
    int Start,
    int Length,
    CommentKind Kind);
