...
internal sealed class GitHubIssueCreator
{
    ...
    private static readonly ConcurrentDictionary<string, byte> LabelCache = new(StringComparer.OrdinalIgnoreCase);
    ...
    public bool TryCreateIssue(TodoItem item, out string gitUrl)
    {
        ...
        var labels = SplitLabels(item.Label).ToArray();
    ...
    {
        if (string.IsNullOrWhiteSpace(label) || LabelCache.ContainsKey(label))
        {
            return;
        }
        ...
        LabelCache.TryAdd(label, 0);
    }

    private static string CreateRandomColour() =>
        RandomNumberGenerator.GetInt32(0x1000000).ToString("X6", CultureInfo.InvariantCulture);
